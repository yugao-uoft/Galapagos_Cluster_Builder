...
def attention_output_0(data_path, json_object, kern_id, kern_dest, kern_src, dest_cluster_per_layer, hidden_size=768, num_attention_heads=12, max_sentence_len=128, encoder_layer=0, cluster_id=0, out_cluster=False):
    ...
    for i in range(json_object['partition']):
        ...
        in_weight_file = open(data_path + '/' + str(encoder_layer) + '/self_output' + '/linear/weight_integer.txt', 'r')
        in_bias_file = open(data_path + '/' + str(encoder_layer) + '/self_output' + '/linear/bias_integer.txt', 'r')
        ...
        start_index_K = 0
        # linear
        linear(in_weight_file, in_bias_file, top_file, param_file, weight_file, config_name, 'in', 'pipe_2', id, dest, M, K, T, P, start_index_M, start_index_K)
        # params for act
        in_m_file = open(data_path + '/' + str(encoder_layer) + '/self_output' + '/act_linear/m.txt', 'r')
        in_e_file = open(data_path + '/' + str(encoder_layer) + '/self_output' + '/act_linear/e.txt', 'r')
...
def attention_output_1(data_path, json_object, kern_id, kern_dest, kern_src, dest_cluster_per_layer, hidden_size=768, num_attention_heads=12, max_sentence_len=128, encoder_layer=0, cluster_id=0, out_cluster=False):
    config = json_object['config']
    for inst in config:
        if inst['name'] == 'act':
            act_config = inst
    ...
    weight_file = open(kern_path + '/src/weights.hpp', 'w')
    in_m1_file = open(data_path + '/' + str(encoder_layer) + '/self_output' + '/act_linear/m1.txt', 'r')
    in_e1_file = open(data_path + '/' + str(encoder_layer) + '/self_output' + '/act_linear/e1.txt', 'r')
    ...
    # params
    in_shift_file = open(data_path + '/' + str(encoder_layer) + '/self_output' + '/layernorm/shift.txt', 'r')
    in_bias_file = open(data_path + '/' + str(encoder_layer) + '/self_output' + '/layernorm/bias.txt', 'r')
    ...
    # params
    in_m_file = open(data_path + '/' + str(encoder_layer) + '/self_output' + '/act_layernorm/m.txt', 'r')
    in_e_file = open(data_path + '/' + str(encoder_layer) + '/self_output' + '/act_layernorm/e.txt', 'r')
    config_name = 'config_t_act_' + str(id)
    # dim
    channel = int(hidden_size)
    start_index = 0
